...
static int transop_encode_sm4 (n2n_trans_op_t *arg,
    ...
    if(in_len <= N2N_PKT_BUF_SIZE) {
        if((in_len + SM4_PREAMBLE_SIZE + SM4_BLOCK_SIZE) <= out_len) {
            ...
            //根据可能不同的SM4_PREAMBLE_SIZE  进行调整
            idx = SM4_PREAMBLE_SIZE;

            // the plaintext data  明文
...
// see transop_encode_sm4 for packet format   数据包格式见transop_encode_sm4
static int transop_decode_sm4 (n2n_trans_op_t *arg,
    ...
    int len = -1;
     ...
     if(((in_len - SM4_PREAMBLE_SIZE) <= N2N_PKT_BUF_SIZE) /* cipher text fits in assembly 密码文本适合汇编*/
      && (in_len >= SM4_PREAMBLE_SIZE)                     /* has at least random number 至少有一个随机数*/
      && (in_len >= SM4_BLOCK_SIZE)) {                     /* minimum size requirement for cipher text stealing 密文窃取的最小尺寸要求*/
        ...
        if(rest) { /* cipher text stealing 密文窃取*/
            ...
            // everything normal up to penultimate block /一切正常到倒数第二块
            memcpy(assembly, inbuf, penultimate_block);

            // prepare new penultimate block in buf准备新的倒数第二块
 	 ...
 	 //  void sm4_crypt_ecb( sm4_context *ctx,int mode, int length, unsigned char *input,unsigned char *output);
	   sm4_crypt_ecb(priv->ctx,0,SM4_BLOCK_SIZE,inbuf+penultimate_block,buf);
            ...
            // former penultimate block becomes new ultimate block//前倒数第二个块 变成了新的块
	      ...
	      sm4_crypt_cbc( priv->ctx,0,in_len + SM4_BLOCK_SIZE - rest,sm4_null_iv,assembly, assembly);		
            ...
            // regular cbc decryption on multiple block-sized payload  //多块大小有效负载上的常规cbc解密 
