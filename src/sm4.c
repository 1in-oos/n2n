 ...
 */
void sm4_setkey_enc( sm4_context_t *ctx, unsigned char key[16] )
{
 ...
 */
void sm4_setkey_dec( sm4_context_t *ctx, unsigned char key[16] )
{
	int i;
	ctx->mode = SM4_ENCRYPT;
	sm4_setkey( ctx->sk, key );
...
void sm4_crypt_ecb( sm4_context_t *ctx,
                    int mode,
                    int length,
                    unsigned char *input,
                    ...
                    unsigned char *output )
{
	...
	if ( mode == SM4_ENCRYPT )
	{
		...
		{
			for ( i = 0; i < 16; i++ )
				output[i] = (unsigned char)( input[i] ^ iv1[i] );
